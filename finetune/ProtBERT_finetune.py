...
def train(args):
    ...
    for epoch in range(args.epochs): # args.epochs
        ...
        if test_mse < test_min_mse:
            ...
            test_min_mse_ktc = test_ktc
            torch.save(model.state_dict(), "prot_bert_finetune_toxicity.pkl")
    ...
    return args.seed, test_min_mse_epoch, test_min_mse, test_min_mse_pcc, test_min_mse_ktc
...
if __name__ == "__main__":
    ...
    for i in range(max_num_split):
        ...
        best_path = "/home/jianxiu/Documents/amp_toxicity_regression/data/repeat1" + str(i) + "best_result.csv"
        best_seed_list, best_mse_epoch_list, best_mse_list, best_mse_pcc_list, best_mse_ktc_list = [], [], [], [], []
        for seed in range(max_num_seed):
            my_seed = random.randint(0, 1000000)
            result_path = "/home/jianxiu/Documents/amp_toxicity_regression/data/" + str(i) + "/result" + str(my_seed) + ".csv"
            ...
            parser.add_argument("--weight_decay", type=float, default=4e-3, metavar="M",
                                help="weight_decay (default: 0.01)")
            parser.add_argument("--epochs", type=int, default=20, metavar="N",
                                help="number of epochs to train (default: 100)")

            best_seed, best_epoch, best_test_mse, best_test_pcc, best_test_ktc = train(parser.parse_args())
            ...
            best_mse_ktc_list.append(best_test_ktc)

        best_result_dict = {"best_seed": best_seed_list, "best_epoch" : best_mse_epoch_list,
                            "best_mse": best_mse_list, "best_mse_pcc": best_mse_pcc_list, "best_mse_ktc": best_mse_ktc_list}
...
# fig = plt.figure()
# ax = fig.add_subplot()
# plt.scatter(test_target_list, test_predict_list, s=10, color="lightblue", edgecolors="black")
# plt.ylabel("Predicted pHD50 values (-log μM)",  size=15)
# plt.xlabel("Experimental pHD50 values (-log μM)",  size=15)
...
# target_predict_dict = {"target":test_target_list, "predict":test_predict_list}
# target_predict_df = pd.DataFrame(target_predict_dict)
# target_predict_df.to_csv("target_predict.csv")
