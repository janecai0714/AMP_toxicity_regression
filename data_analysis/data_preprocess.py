...
data_df_ave = data_df_ave.rename(columns = {'HEMOLITIC CYTOTOXIC ACTIVITY - CONCENTRATION - PROCED' : 'HD50'})
data_df_ave['SEQUENCE_space'] = [' '.join(ele) for ele in data_df_ave['SEQUENCE']]
data_df_ave['pHD50'] = -np.log10(data_df_ave['HD50'])
data_df_ave['LENGTH'] = data_df_ave['SEQUENCE'].str.len()
data_df_ave.to_csv("hemolysis50_ave_no_limit.csv", index=False)

data_df_ave = data_df_ave.loc[(data_df_ave['LENGTH'] >= 5) & (data_df_ave['LENGTH'] <= 50)] # only few seqs with length 5, and 4 seqs with length more than 50
data_df_ave = data_df_ave.loc[(data_df_ave['pHD50'] >= -3) & (data_df_ave['pHD50'] <= 1)] # min -3, max 1 with few instances
